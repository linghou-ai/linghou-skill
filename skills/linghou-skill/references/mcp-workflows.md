...
- 找 tab：`command: "tabs_list"`，不传 `tabId`。
- 只读页面内容：`command: "get_page_markdown_with_iframes"` + `tabId`。
- 页面交互：先 `command: "snapshot"` + `tabId`，再用返回的 Ref 调 `click`、`input` 等命令。
- 内置命令不能完成时，才使用 `command: "evaluate"` + `scriptCode`；需要复用或受治理的脚本，使用 `commandSlug`。
- 不要猜测 command；完整目录见 `command-catalog.md`。
